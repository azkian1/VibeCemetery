...
### 0.3. Ближайший этап и разделение задач между агентами

Работа выполняется последовательно только в защищённом Vercel Preview.
Production deployment, production flags и домен `vibecemetery.app` не
изменяются. Реальная подпись транзакции и перевод GRAVE запрещены до отдельного
подтверждения владельца.

#### Пункт 1 — Preview UI и Vercel

Ответственный агент проверяет и выполняет только Preview-настройку:

1. сохранить `NEXT_PUBLIC_WEB3_GRAVE_BURNS_ENABLED=false` в Production;
2. создать или проверить Preview-only значение
   `NEXT_PUBLIC_WEB3_GRAVE_BURNS_ENABLED=true`;
3. не изменять Preview server flag, RPC URL и server-only secrets;
4. передеплоить последний commit ветки `codex/burn-v1-only`;
5. подтвердить `Ready` и отсутствие `4xx`/`5xx` в runtime logs;
6. проверить наличие панели только в модалке реальной могилы Map v1;
7. подтвердить отсутствие панели на Map v2, meta grave и пустом слоте.

Критерий приёмки: UI доступен только пользователям защищённого Preview, а
Production остаётся скрыт и выключен.

#### Пункт 2 — закрытый server/API smoke

Ответственный агент не создаёт реальную burn-транзакцию и не отключает Vercel
Authentication:

1. проверить server config fail-closed и Base Mainnet chain id `8453`;
2. выполнить same-origin негативный API smoke, который гарантированно не может
   создать intent или burn запись;
3. ожидать контролируемый `400` от приложения, а не `503` конфигурации;
4. при невозможности пройти Vercel Authentication не ослаблять защиту, а
   зафиксировать точную границу проверки;
5. подтвердить в Supabase `grave_burn_intents = 0` и `grave_burns = 0`.

Критерий приёмки: серверный контур доступен в Preview, запрос не меняет базу,
секреты не попадают в вывод или browser bundle.

#### Пункт 3 — независимая QA-проверка

Ответственный агент не меняет Production и не выполняет внешний write:

1. запустить 42 профильных burn/security unit-теста;
2. запустить TypeScript и ESLint;
3. запустить fake-wallet Web3 E2E без реального RPC transfer;
4. проверить `git diff --check` и чистоту рабочей ветки;
5. сопоставить фактические Preview flags, deployment и документацию;
6. составить список блокеров отдельно от старых baseline-предупреждений.

Критерий приёмки: все профильные gates зелёные, новых регрессий нет, а
результат можно воспроизвести без кошелька пользователя.

#### Действие владельца после агентов

После успешных трёх пунктов владелец открывает Preview, выбирает реальную
могилу v1 и подключает MetaMask или Rabby. На этом этапе проверяется только
