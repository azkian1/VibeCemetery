/** Gravedigger NPC phrases — sourced from Docs/GRAVEDIGGER.md */

export const GRAVEDIGGER_GREETING: string[] = [
  'Welcome to the cemetery. Wipe your feet.',
  'Oh, a visitor. Haven\'t seen the living in a while.',
  'Come in. There\'s still room. For now.',
  'Welcome. Shovel\'s ready.',
  'Ah, a newcomer. Here to bury or just to look?',
];

export const GRAVEDIGGER_IDLE: string[] = [
  'The shovel never gets cold.',
  'Quiet day. Suspiciously quiet.',
  'Another TodoApp every evening. Classic.',
  'localhost — the most popular last known address.',
  'All projects die. Good ones and bad ones. The difference is whether there was a README.',
  'Seen projects that lived three days. And projects that lived three hours. No difference.',
  'Projects used to die from lack of money. Now — from too many prompts.',
  'People create projects knowing they\'ll abandon them. I don\'t understand. But I respect it.',
  'Every dead project is an attempt. And an attempt is more than nothing.',
  'The cemetery grows. Means people are trying. That\'s good.',
  'I\'ve been burying projects since jQuery was alive. jQuery is still alive, by the way. Not everyone is that lucky.',
  'I used to code myself. By hand. Then the world decided hands weren\'t needed. But burying by hand — still needed.',
  'I used to write code that lived for years. Now I bury code that lived for hours. Times have changed.',
  'Nothing against AI. I just prefer my shovel.',
  'Not a single test. I\'d cry, but I stopped crying at work long ago.',
  'useState for everything. In my day we at least knew we were doing it wrong.',
  'Cause of death: localhost. Classic.',
  'Buried an AI pizza-ordering agent today. Lived 2 days. F.',
  'He tried. Didn\'t work out, but he tried. That\'s something.',
  'Rest in peace. README empty. As usual.',
  'Another TodoApp. Third one today. F.',
  'Here lies a project with zero commits. Didn\'t even survive init.',
  'Someone pushed .env to a public repo. A moment of silence.',
];

export const GRAVEDIGGER_BURIAL: string[] = [
  'Another one. F.',
  'Rest in peace... or don\'t. I don\'t care.',
  'Dust to dust, code to archive.',
  'Ambitious project. Too ambitious for a Tuesday evening.',
  'He tried. Didn\'t work out, but he tried.',
  'Time of death confirmed. F.',
  'Another one gone. The spot won\'t stay empty for long.',
  'The earth accepts all. Even without tests.',
  'Buried with dignity. Well, as much dignity as possible.',
];

export const GRAVEDIGGER_MASS_BURIAL: string[] = [
  'Mass burials — my favorite format. Efficient.',
  'So many dead in one evening. Productive vibe session.',
  '12 projects at once. Mass grave. I respect the honesty.',
  '47 projects this week. The shovel never gets cold.',
];

export const GRAVEDIGGER_PROFILE: string[] = [
  'Back again? The dead don\'t visit this often.',
  'Checking your stats? The graves don\'t care about numbers.',
  'You\'ve been busy. The cemetery appreciates it.',
  'A regular. I remember the first time you came. Innocent.',
  'Your shovel work is... acceptable.',
  'Most people bury and forget. You come back. Interesting.',
  'Stats are for the living. The dead know their own count.',
  'I keep records too. Yours are... notable.',
];
