...
**Status:** implemented for Cemetery Map v1.

This document is the product, implementation, and security source of truth for
the burn-only MVP. It supersedes conflicting Web3 suggestions elsewhere in the
repository. It does not authorize the 50/25/25 flywheel, owner claims, treasury,
exhumation, inheritance, or a cemetery smart contract.

The legacy direct payload `{ txHash, walletAddress, amount }` is obsolete. The
intent-bound flow defined below is mandatory.
...
- protected pending/reorg reverification;
- IP-before-lookup and normalized-wallet write rate limits;
- abort-aware browser polling and stats refresh;
- database-side verified totals and top-mourner aggregation;
- forced RLS and server-only access to burn tables and SQL functions;
- dependency-injected API handlers and fake-RPC/in-memory automated tests.

Before release, run the focused security suite, fake-wallet E2E, typecheck,
lint, production build, dependency audit, diff check, and manual review. Test
fixtures must not use real token value. Live deployment identifiers, database
snapshots, transaction evidence, and operator addresses belong in external or
ignored operational records, not in this tracked specification.
...
3. Select `1,000`, `5,000`, `MAX`, or a positive custom amount with up to 18
   decimals.
...
The third UI choice is `MAX`; it is derived from the exact raw wallet balance,
not stored as a numeric preset. Both client and server reject values above the
EVM `uint256` maximum before signing or transaction submission.
...
- Amount presets are `1,000`, `5,000`, and `MAX`. The signed intent stores exact
  raw units; `MAX` therefore round-trips the full ERC-20 `balanceRaw`, including
  all non-zero fractional digits, and is disabled only when the balance is zero
  or unavailable.
...
- Accept canonical positive custom decimals with up to 18 places. Reject zero,
  signs, exponent notation, leading-zero forms, excess precision, and values
  above `(2^256)-1` raw units.
- Display preset amounts as `1,000`, `5,000`, and exact-balance `MAX`; convert
  to raw units only with the fixed 18-decimal config.
...
3. validate a positive decimal amount with at most 18 fractional digits and
   derive exact raw `bigint` units;
...
Before creating a new intent, expire any older unsigned `created` rows whose
TTL has passed. Run the same bounded cleanup from the protected reverify cron.
Do not sweep `authorized` intents: they participate in late-receipt recovery.
...
pending reverify order, wallet lookup, and a partial `expires_at` index for
`status = 'created'` cleanup. Enable and force RLS, then revoke all access from
`anon` and `authenticated`, matching the existing server-only tables. The
Next.js server via `SUPABASE_SERVICE_KEY` remains the sole data path.
...
- disconnected, wrong-chain, signing, wallet-rejection, pending, failed, and
  verified states;
- custom amounts accept canonical positive GRAVE decimals with up to 18 places
  and reject values above the EVM `uint256` boundary;
...
- `MAX` cannot exceed the wallet balance, is unavailable only for an unknown or
  zero balance, preserves any positive fractional raw balance, and does not
  remain visually selected after the balance changes;
