...
test('wallet provider is scoped to CemeteryAppV2 and absent from global providers', () => {
...
test('shared grave modal renders the burn panel only for a real v2 grave', () => {
  const modal = fs.readFileSync(path.join(root, 'src/components/modals/GraveModal.tsx'), 'utf8')
  expect(modal).toContain("mapVersion === 'v2' && slotId != null")
...
test('feature flag exits before any Wagmi hook executes', () => {
  ...
  const guard = panel.indexOf("if (!WEB3_GRAVE_BURNS_VISIBLE || mapVersion !== 'v2') return null")
