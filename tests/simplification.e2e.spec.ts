...
test('REKT grave preview shows public content and keeps actions local', async ({ page }, testInfo) => {
  await fixtures(page, 'v2')
  const writes: string[] = []
  page.on('request', request => { if (request.method() === 'POST') writes.push(request.url()) })
  await page.goto('/dev/rekt?view=grave')
  const card = page.getByRole('article', { name: 'REKT memorial' })
  await expect(card.getByRole('heading', { name: '$EXAMPLE', exact: true })).toBeVisible()
  await expect(card.getByText('−$8,000.00', { exact: true })).toBeVisible()
  await expect(card.getByText('Buried by 0x1111…1111', { exact: true })).toBeVisible()
  expect(await card.innerHTML()).not.toContain('0x' + '1'.repeat(40))
  await expect(page.getByText('This memorial will be public.', { exact: true })).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'Bury REKT', exact: true })).toHaveCount(0)
  await page.screenshot({ path: testInfo.outputPath('rekt-grave-desktop.png'), fullPage: true })
  await page.setViewportSize({ width: 375, height: 812 })
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
  await expect(card.getByRole('button', { name: 'Find on Map', exact: true })).toBeInViewport()
  await page.screenshot({ path: testInfo.outputPath('rekt-grave-mobile.png'), fullPage: true })
  await card.getByRole('button', { name: 'Press F', exact: true }).click()
  await expect(card.getByText('13', { exact: true })).toBeVisible()
  await expect(card.getByRole('button', { name: 'F ✓', exact: true })).toBeDisabled()
  await card.getByRole('button', { name: 'Burn GRAVE', exact: true }).click()
  await expect(card.getByRole('status')).toContainText('No wallet opens')
  await card.getByRole('button', { name: 'Share Grave', exact: true }).click()
  await expect(card.getByRole('status')).toContainText('no public grave link')
  await card.getByRole('button', { name: 'Find on Map', exact: true }).click()
  await expect(card.getByRole('status')).toContainText('no plot on the map')
