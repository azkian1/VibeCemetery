...
export const WHOLE_GRAVE_AMOUNT_RE = /^[1-9][0-9]*$/
...
export function parseWholeGraveAmount(value: unknown): {
  amountWhole: string
  amountRaw: bigint
} | null {
  if (typeof value !== 'string' || !WHOLE_GRAVE_AMOUNT_RE.test(value)) return null
  if (value.length > 60) return null

  try {
    const amountRaw = parseUnits(value, GRAVE_TOKEN_DECIMALS)
    if (amountRaw <= 0n || amountRaw > MAX_NUMERIC_78) return null
    return { amountWhole: value, amountRaw }
  } catch {
    return null
  }
}
