...
Two of nine planned zones are open. Future zones will be released around announced GRAVE burn checkpoints. The runtime does not yet assign stable zone IDs or automatically unlock terrain from a burn threshold. The [master-plan illustration](images/cemetery-master-plan.png) labels the 144 current plots as A: 60 and D: 84, divided at world y = 2312 by plot center for the illustration. Its seven future-zone allocations remain provisional and sum to 522.
...
Every live grave retains its persisted GID and approved slot footprint. The visual is selected by `src/game/utils/paintedArtV2.ts`; placement, input and links stay attached to the original slot. All 47 painted grave frames use one horizontal and vertical scale, measured from each frame's visible alpha bounds in `paintedGraveBoundsV2.json`. The visible pedestal is centered on the slot and its bottom rests two pixels below the slot's lower edge. Tall, wide and large graves have separate maximum visual envelopes, but wide upright monuments may rise above their shallow 64×32 ground footprint. This preserves each template's geometry instead of squeezing it into the slot rectangle. The two top-down wide models, broken keyboard and GPU, have upright repaints in atlas 07. The non-square source atlas 01 is assembled frame by frame without distorting its four stones. Run `node scripts/prepare-open-art.mjs` after changing any source grave art; it rebuilds atlas 07, the WebP atlases and the measured bounds. Run `node scripts/audit-painted-graves.mjs` to produce contact sheets of all 47 variants at game scale. Tree sprites are complete under the fog. Their trunks are checked against approved slots, while crowns can overlap graves. Three tree roots near or on paving were moved onto grass; `node scripts/audit-painted-trees.mjs` verifies the visible roots against the active terrain. The six legacy tree objects with trunks on approved slots are omitted visually. The fog stays above all art.

All 47 grave models passed the local review recorded in `artifacts/grave-review/approved-final.json`. The 23 angle corrections live as individual source PNGs in `artifacts/painted-map-sources/grave-redraws/` and as separate runtime WebP atlases named `grave-redraw-atlas-XX.webp`. `paintedGraveRedrawIdsV2.json` selects those frames; the original atlases continue to serve the other 24 models unchanged. `node scripts/build-reviewed-grave-atlases.mjs` rebuilds the corrected atlases, and `node scripts/build-grave-art-bounds.mjs` remeasures every visible silhouette. Run `node scripts/export-grave-review.mjs` and `node scripts/verify-grave-review.mjs` to check the approved frames against their recorded hashes.
