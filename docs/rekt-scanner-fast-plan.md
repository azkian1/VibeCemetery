...
## CU accounting and load limits (2026-09-12)

The PnL CLI now defaults to **450 throughput CU/s** and **50,000 estimated RPC CU per invocation**, including retry attempts. Configure `--cu-per-second` / `--max-cu` or `REKT_CU_PER_SECOND` / `REKT_MAX_CU`; CLI flags take precedence. These defaults also apply to `npm run rekt:fast`. For example, `--cu-per-second 300 --max-cu 20000` leaves more capacity for another project. Values must be positive safe integers; unknown RPC method costs fail closed when limits are active.

RPC requests across every host/chain in one scan share a serialized, weighted, no-burst launch gate. Each request waits at least its throughput weight divided by the configured rate, or the existing request interval, whichever is longer. Billing and throughput weights are separate (`eth_chainId`: 0 billing CU, 5 throughput CU). REST requests to Relay/pricing providers retain per-origin pacing and count toward the HTTP request budget, with no invented Alchemy CU cost. Batch payloads are unsupported and rejected before dispatch.

Success and failure reports include `metrics.usage`: attempted requests and estimated CU by method/provider, `estimatedRpcCu` (all RPC providers using Alchemy-equivalent weights), `estimatedAlchemyCu` (recognized Alchemy RPC hosts only), throughput CU, retries, HTTP 429 and JSON-RPC throttle counts, and completed pacing waits in `pacingWaitMs`. The latter sums scheduler delays; it is not elapsed wall time, excludes retry backoff and can overlap external-provider waits. `metrics.limits` records the applied settings. Existing `cacheReceiptsReused` remains available on completed discovery reports. Endpoints, credentials and request parameters are not recorded in usage metrics.

The cost table is pinned to [Alchemy's official reference](https://www.alchemy.com/docs/reference/compute-unit-costs), checked 2026-09-12. These are **attempt-based estimates, not billing**: failed attempts are counted conservatively; provider rules may differ. Custom proxy hosts are classified as `other_rpc`. Without active CU limits, unknown RPC methods are explicitly counted as unknown with a 120 CU fallback estimate. This patch configures the PnL CLI; the separate funding CLI retains its existing request limits.

On `cu_budget_exhausted`, workers drain and the checkpoint is retained wherever scan state has been initialized. Continue with `--resume`; every invocation starts a new usage counter and CU budget. This is not a monthly spending cap. Keep each report to sum multiple runs. No monthly usage is inferred from the Alchemy dashboard.

**Scope:** one Transport/CLI process. Different wallets launched as separate processes, other projects, and other server instances are not coordinated. Until a shared server queue/limiter is implemented, run scans one at a time and reserve capacity for other projects. Alchemy's account-wide limit includes their traffic too; see [throughput documentation](https://www.alchemy.com/docs/reference/throughput). Raising the CLI setting does not change the provider's quota.

Earlier 21.762/21.697-second benchmarks predate weighted CU pacing. Latency with the new 450 CU/s limit must be measured after quota is available. Local tests use mocked providers and do not spend Alchemy quota.
...
- Parallel workers are bounded at 1–16 (fast command: 10). RPC spacing is shared across origins with weighted CU pacing; external REST spacing is per origin. Retry attempts share the request and CU budgets. Workers drain before error reporting, pool lookups are deduplicated while in flight, and checkpoint writes are coalesced and serialized.
