...
test('Crematory building opens supply ledger and remains readable on mobile', async ({ page }, testInfo) => {
  ...
  await fixtures(page, mapVersion)
  await page.goto(mapPath)
  await expect(page.getByTestId(mapVersion === 'v1' ? 'phaser-stage' : 'phaser-stage-v2')).toHaveAttribute('data-scene-ready', 'true')
  await page.waitForTimeout(2100) // Initial camera zoom lasts 2000 ms.
  const point = mapVersion === 'v1'
    ? projectWorldPoint(createMinimapProjection(1920, 1920, 140), 700, 200)
    : projectWorldPoint(createMinimapProjection(4480, 3328, 140), 3008, 2800)
  ...
  await expect(async () => {
    await page.mouse.click(mapVersion === 'v1' ? 640 : 1130, mapVersion === 'v1' ? 147 : 480)
...
test('the development branch serves the selected cemetery map', async ({ page }) => {
  await fixtures(page, mapVersion)
...
test('HUD and FAQ describe graves, the shared allowance and token tributes', async ({ page }) => {
  ...
  await expect(page.getByRole('region', { name: 'How many graves do I get?' })).toContainText('share this allowance across both maps')
...
test('v1 minimap survives late mounting and the red BURY button sits beside the bottom chat', async ({ page }, testInfo) => {
  ...
  await expect.poll(terrainPixels).toBeGreaterThan(2000)
