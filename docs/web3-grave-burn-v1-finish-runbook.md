# GRAVE Burn — preflight, hardening и Preview runbook

Имя файла сохранено для совместимости со ссылками первоначального v1-релиза.
Миграции и проверки burn применимы к обеим картам в текущей development-ветке;
публикация v2 остаётся отдельным действием. Актуальная механика и порядок
hash-recovery migration описаны в `web3-grave-burn-mvp.md`.
Этот runbook не запускает deployment, SQL или Base Mainnet транзакции автоматически.

## 1. Что является релизной единицей

Для этой burn-механики не нужен новый Solidity-контракт. Релиз состоит из четырёх частей:
