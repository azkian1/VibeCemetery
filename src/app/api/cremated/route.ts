...
export const CREMATED_MAX_BODY_BYTES = 16 * 1024
export const CREMATED_WRITE_RATE_LIMIT = 60
export const CREMATED_WRITE_RATE_WINDOW_MS = 60_000
const DEFAULT_CREMATED_LIST_LIMIT = 100
const MAX_CREMATED_LIST_LIMIT = 100
const MAX_CREMATED_LIST_OFFSET = 10_000
const GITHUB_USERNAME_PATTERN = /^[a-z0-9](?:[a-z0-9-]{0,37}[a-z0-9])?$/

type CrematedBodyReadResult =
...
export function isCrematedBodyTooLarge(request: Request): boolean {
  const contentLength = getDeclaredContentLength(request)
  return contentLength !== null && contentLength > CREMATED_MAX_BODY_BYTES
}
...
/**
 * Reads a JSON body without trusting Content-Length alone. The stream cap also
 * protects against absent or forged length headers before JSON.parse runs.
 */
export async function readCrematedJsonWithLimit(request: Request): Promise<CrematedBodyReadResult> {
  if (isCrematedBodyTooLarge(request)) {
    return { ok: false, status: 413, error: 'Request body too large' }
  }

  const reader = request.body?.getReader()
  if (!reader) {
    return { ok: false, status: 400, error: 'Invalid or malformed request body' }
  }
  ...
  try {
    while (true) {
      ...
      if (totalBytes > CREMATED_MAX_BODY_BYTES) {
        // Do not keep reading an oversized stream. Avoid awaiting cancellation
        // because a hostile/custom stream could make its cancel handler hang.
  ...
  try {
    const text = new TextDecoder().decode(joinBodyChunks(chunks, totalBytes))
...
export async function checkCrematedWriteRateLimit(request: NextRequest) {
  return checkRateLimit(
    `cremated-write:${getClientIp(request)}`,
    CREMATED_WRITE_RATE_LIMIT,
    CREMATED_WRITE_RATE_WINDOW_MS,
  )
}

export function crematedWriteRateLimitResponse(retryAfterMs: number): NextResponse {
...
export type CrematedListQuery =
  | { ok: true; author: string | null; limit: number; offset: number }
...
export function parseCrematedListQuery(searchParams: URLSearchParams): CrematedListQuery {
  const rawAuthor = searchParams.get('author')
  let author: string | null = null

  if (rawAuthor !== null) {
    author = rawAuthor.trim().toLowerCase()
    if (!GITHUB_USERNAME_PATTERN.test(author)) {
  ...
  return {
    ok: true,
    author,
    limit: parseBoundedInteger(searchParams.get('limit'), DEFAULT_CREMATED_LIST_LIMIT, 1, MAX_CREMATED_LIST_LIMIT),
    offset: parseBoundedInteger(searchParams.get('offset'), 0, 0, MAX_CREMATED_LIST_OFFSET),
  }
}
...
export async function POST(request: NextRequest) {
  ...
  // This check is intentionally before rate limiting and auth: it is O(1),
  // avoids expensive work for obviously oversized requests, and does not read the body.
  if (isCrematedBodyTooLarge(request)) {
    return NextResponse.json({ error: 'Request body too large' }, { status: 413 })
  }

  const writeRateLimit = await checkCrematedWriteRateLimit(request)
  if (!writeRateLimit.allowed) {
    return crematedWriteRateLimitResponse(writeRateLimit.retryAfterMs)
  }
  ...
  const bodyResult = await readCrematedJsonWithLimit(request)
  if (!bodyResult.ok) {
...
export async function GET(request: NextRequest) {
  const listQuery = parseCrematedListQuery(request.nextUrl.searchParams)
  if (!listQuery.ok) {
  ...
  if (listQuery.author) {
    query = query.ilike('author_github', listQuery.author)
  }
