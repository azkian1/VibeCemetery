...
export default function PhaserCanvasV2() {
  ...
  const handleBuildingClick = useCallback((data: SlotEventData) => {
    const modal: ModalType | null =
      data.name === 'Crematory' ? 'crematory'
      : data.name === 'Chapel' ? 'mausoleum'
      : data.name === 'Gravedigger Lodge' ? 'caretaker'
      : null;
    if (!modal) return;
    dispatch({
      type: 'OPEN_MODAL',
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { createGameConfigV2 } = await import('../game/config-v2');
      ...
      const game = new Phaser.Game(createGameConfigV2(parent, initialSize));
