...
test('injected wallet completes a stubbed verified burn offering', async ({ page }) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname === '/api/graves' && request.method() === 'GET') {
      return json([{
        id: graveId,
        slot_id: getAutoAssignableGraveSlots(mapVersion)[0].id,
        grave_gid: null,
  ...
  await page.goto(`${mapPath}?grave=${graveId}`)
  ...
  const tenThousand = page.getByRole('button', { name: 'Offer 10,000 GRAVE' })
  const maximum = page.getByRole('button', { name: /Offer maximum/ })
  await expect(preset).toHaveAttribute('aria-pressed', 'true')
  await expect(page.getByLabel('Custom GRAVE amount')).toHaveCount(0)
  await tenThousand.click()
  await expect(tenThousand).toHaveAttribute('aria-pressed', 'true')
