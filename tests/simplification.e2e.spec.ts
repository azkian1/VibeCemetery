...
const slot = getAutoAssignableGraveSlots('v2')[0]
const grave = { id, name: 'Local project', cause: 'Lost interest', source: 'local', author_github: 'tester',
  slot_id: slot.id, grave_gid: pickGraveGidV2(slot.type, 0), map_version: 'v2', github_url: null, github_repo_id: null,
...
async function fixtures(page: Page, options: { authenticated?: boolean; slotsUsed?: number; failLedger?: boolean; holdWrite?: Promise<void> } = {}) {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (url.pathname === '/api/graves' && req.method() === 'GET') return json(url.searchParams.get('map_version') === 'v1' ? [] : [grave])
...
test('Necropolis recovers from a ledger error and shows exact received offerings', async ({ page }, testInfo) => {
  const { recoverLedger } = await fixtures(page, { failLedger: true })
  await page.goto('/cemetery/v2')
...
test('approved account submits one burial to the current map and guards a pending write', async ({ page }) => {
  ...
  const { writes } = await fixtures(page, { authenticated: true, slotsUsed: 3, holdWrite })
  ...
  expect(writes[0]).toMatchObject({ source: 'github', map_version: 'v2', github_repo_id: 42, cause: 'Developer lost interest' })
  release()
...
test('shared account limit disables the grave action without offering cremation', async ({ page }) => {
  await fixtures(page, { authenticated: true, slotsUsed: 4 })
  await page.goto('/cemetery/v2')
...
test('local grave deep link opens normal epitaph and offerings without a GitHub link', async ({ page }) => {
  await fixtures(page)
  await page.goto('/cemetery/v2?grave=' + id)
...
test('Crematory building opens supply ledger and remains readable on mobile', async ({ page }, testInfo) => {
  ...
  await expect(async () => {
    await page.mouse.click(1056, 416)
