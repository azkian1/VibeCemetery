...
test('local HTTP request produces a real grave, server epitaph, map sprite and private identity', async () => {
  ...
  expect(grave).toMatchObject({ source: 'local', map_version: 'v2', author_github: 'tester', github_url: null, github_repo_id: null })
  expect(grave.epitaph).toBeTruthy()
  expect(grave.epitaph).not.toBe('FORGED')
  expect(grave.grave_gid).toBeGreaterThan(0)
  expect(getAutoAssignableGraveSlots('v2').some(s => s.id === grave.slot_id)).toBe(true)
...
test('a full map rejects a new project but still recovers the original grave', async () => {
  const first = await (await request(local())).json()
  const slots = getAutoAssignableGraveSlots('v2').filter(s => s.id !== first.slot_id)
