...
test('v2 minimap represents each authored fog-of-war state', () => {
  ...
  expect(fog[49 * map.width + 74]).toBe(3)
  expect(fog[50 * map.width + 74]).toBe(2)
  expect(fog[52 * map.width + 74]).toBe(1)
  expect(fog[53 * map.width + 74]).toBe(0)

  expect(fog.filter((state) => state === 3)).toHaveLength(11802)
  expect(fog.filter((state) => state === 2)).toHaveLength(177)
  expect(fog.filter((state) => state === 1)).toHaveLength(245)
  expect(fog.filter((state) => state === 0)).toHaveLength(2336)
})
