...
test('v2 camera wires the real locked-fog mask into drag while zoom and minimap stay strict', () => {
  ...
  expect(minimapHandler).toContain('cameraScrollForCenter(cam, data.worldX, data.worldY)')
  expect(minimapHandler).toContain('Phaser.Math.Clamp(target.x, bounds.minX, bounds.maxX)')
  expect(minimapHandler).toContain('Phaser.Math.Clamp(target.y, bounds.minY, bounds.maxY)')
