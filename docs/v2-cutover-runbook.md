...
Status: v2 and the v1-retirement commit are live in production with 144 active slots. All nine old graves and the synthetic production burial are verified. V1 runtime/public assets are removed; Storage v1 is private. Read-only observation runs every four hours through 2026-09-10T17:36:38.438Z. The ten private Storage objects remain; final deletion is deferred pending separate approval.
...
The ten private Storage objects remain during the recovery window. No Storage object deletion has occurred. The original goal remains incomplete until the observation window and remaining retirement work are resolved.

### Retirement release and observation

- Published retirement commit `accad8b36d669b45f5f08c98e8271dc7175c333c` to Preview, manually checked its test memorial, the old shotr UUID/redirect with repeated query values, FAQ, chat collapse/expand, restored map input and minimap camera movement. Inspected assets contained no v1 requests and the console was clean.
- Fast-forwarded `master` to the same tested commit. [Production deployment](https://vercel.com/azats-projects-db37144a/vibecemetery/ByVLGJEsScTkRMcTvg9WJRWNpUgk) is Ready and assigned to `vibecemetery.app`; [matching Preview](https://vercel.com/azats-projects-db37144a/vibecemetery/3fT1mmUjfQJZaB2FwKzwgGf6zLrp) is Ready. No force push was used.
- Production read-only verification at `2026-09-07T17:36:38.436Z` passed 116 HTTP checks: all ten UUID share pages and per-grave burn endpoints; original memorial fields; all F counts reconciled to eight vote rows; four verified burns and the global ledger reconciled to `25263442113724649798733865`; compatible unique v2 slots; no v1 database rows; open v2-only gate; aliases/invalid links; retired public files returning 404; all ten public Storage URLs returning 400.
- Fetched the v2 TMJ and all 74 referenced images with fresh query parameters and compared SHA-256. TMJ bytes match the published Git blob; the Windows working copy differs only by CRLF line endings. This is cold HTTP verification, not a claim that a reused browser profile had an empty cache.
- Repeated manual production checks after deployment: test memorial at desktop/mobile, profile (2/5 used, three available), Necropolis, author Crypt, myvibe (F=1; 7,219,522 whole GRAVE), closing the complete nested modal stack, chat collapse/expand and minimap movement. Input returns to `auto`; chat messages persist. Mobile content fits within 390px and inspected browser logs are empty.
- The new deployment's visible Vercel runtime logs show successful grave/account/ledger requests and the expected 404/410 negative checks, with no observed 5xx. Supabase API Gateway recorded expected 400 denials from the controlled legacy Storage probes. The current billing cycle began on September 7: uncached egress ~0.001/5 GB, cached egress ~0.008/5 GB, Storage size ~0.008/1 GB; usage refresh can lag. Product-specific observability navigation was disabled, so a reliable Database-vs-Storage egress split was not established at this checkpoint. Check available usage breakdowns during observation. The old grace-period banner remains, but displayed usage is below current quotas; no billing changes were made.
- Read-only heartbeat automation `v2` (Наблюдение за v2 кладбищем) is Active every four hours, with final verification after `2026-09-10T17:36:38.438Z` (20:36 Moscow). It runs the reviewed local `.local-archive/verify-retirement.mjs` and stores evidence under the private `observation/` directory. It must report unavailable checks or gaps, remain quiet when unchanged, and stop after the final report. The local machine and desktop app must remain running for scheduled local checks.
- Automatic approval review rejected the first proposed automation because it included future irreversible Storage deletion and conditional database gate writes without separate consent to those background side effects. Replaced it with a strictly read-only automation, which was accepted. It cannot delete objects or change the production gate. Obtain separate consent for those actions if needed; do not work around that rejection. Local archives and all ten private Storage objects remain intact.
- Reverify remains scheduled daily at 03:00 UTC in `vercel.json`. The next scheduled job and the full 72-hour trend have not yet been observed. Do not mark this initial checkpoint as a completed observation period.
